...
export const retryConfiguration = (scenario: cucumber.GherkinDocument.Feature.IScenario): RetryConfiguration => {
    const retryTag = scenario?.tags?.find(tag => /^@Retry=/.test(`${tag.name}`))
    if (!retryTag) return defaultRetryConfig
    return retryTag?.name?.split('=')[1].split(',').reduce((settings, config) => {
